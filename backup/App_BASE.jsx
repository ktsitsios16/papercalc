    ...
    <div className="container">
      <div className="header">
        ...
        <div className="lang-switch">
          <button onClick={() => setLang("gr")} className={lang === "gr" ? "active" : ""}>GR</button>
          <button onClick={() => setLang("en")} className={lang === "en" ? "active" : ""}>EN</button>
        </div>
      </div>

      <div className="tabs">
        <button onClick={() => setTab("sheets")} className={tab === "sheets" ? "active" : ""}>{tr.sheets}</button>
        <button onClick={() => setTab("price")} className={tab === "price" ? "active" : ""}>{tr.price}</button>
      </div>

      {tab === "sheets" && (
        <>
          <div className="mode-switch">
            <button onClick={() => setSheetMode("fromWeight")} className={sheetMode === "fromWeight" ? "active" : ""}>Ποσότητα Φύλλων</button>
            <button onClick={() => setSheetMode("fromSheets")} className={sheetMode === "fromSheets" ? "active" : ""}>Βάρος Συσκευασίας</button>
          </div>
          ...
          <div className="result-box">
            {sheetMode === "fromWeight" && (
              <>
                ...
                <div className="small">{tr.totalWeightLabel}: {calcTotalWeight(sheets, activeGsm, activeW, activeH).toFixed(2)} kg</div>
              </>
            )}

            {sheetMode === "fromSheets" && (
              <>
                <div className="big">{totalWeightFromSheets.toFixed(2)} kg</div>
                <div className="small">{tr.sheetWeightLabel}: {sheetWeight.toFixed(2)} g</div>
              </>
            )}
          </div>
      ...
      {tab === "price" && (
        <>
          <div className="mode-switch">
            <button onClick={() => setMode("kgToSheet")} className={mode === "kgToSheet" ? "active" : ""}>€/kg → sheet</button>
            <button onClick={() => setMode("sheetToKg")} className={mode === "sheetToKg" ? "active" : ""}>sheet → €/kg</button>
          </div>
          ...
          <div className="result-box">
            <div className="big">
              {mode === "kgToSheet"
                ? `${priceSheet.toFixed(3)} €/${tr.sheetsResult}`
